...
public:
    ...
    enum SliderPercent {
        ...
        SliderPercent_0_8 = 8,
        SliderPercent_0_9 = 9,
        SliderPercent_1_0 = 10
    };

    explicit SliderRange(QWidget *parent = 0);
    explicit SliderRange(int leftVal, int rightVal, QWidget *parent = 0);
    ~SliderRange();
...
private:
    ...
    bool rangPressed;               //区间指示器是否按下
    QPoint rangPressedPos;          // 按下区间局域坐标

    int sliderLen;                  //指示器长度
    int sliderHeig;                 //指示器高度
    QRect leftSliderRect;           //左边值指示器区域
    QRect rightSliderRect;          //右边值指示器区域
    QRect rangSliderRect;           //区间值指示器区域
